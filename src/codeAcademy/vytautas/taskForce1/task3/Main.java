...
class Main {
    ...
    public static void main(String[] args) throws InvalidShopItemPrice {
       /* Shop shop1 = new Shop("Jonavos g. 60", 150, TradeNetwork.MAXIMA);
        ...
        Shop shop1 = new Shop("Jonavos g. 60", 150, TradeNetwork.MAXIMA,
                new ArrayList<Item>(Arrays.asList(new Item("Banana", -2.0), new Item("Coconut", 2.0))));
        Shop shop2 = new Shop("Panemumes g. 45", 45, TradeNetwork.IKI,
                new ArrayList<Item>(Arrays.asList(new Item("Spices", -0.75), new Item("Plate", 1.4))));
        Shop shop3 = new Shop("Gelezinio Vilko g. 145C", 35, TradeNetwork.RIMI,
                new ArrayList<Item>(Arrays.asList(new Item("IceCream", 1.2), new Item("Cabbage", -2.1))));
        Shop shop4 = new Shop("Pramones g. 6B", 80, TradeNetwork.LIDL,
                new ArrayList<Item>(Arrays.asList(new Item("Frozzen Pizza", 2.5), new Item("Hachet", -25.0))));

        ArrayList<Shop> shopArrayList = new ArrayList<>(Arrays.asList(shop1, shop2, shop3, shop4));
        for (Shop shopTemp : shopArrayList
        ) {
            System.out.println("");
            shopTemp.cheapestItemInTheShop();
        }

        for (Shop shopTemp: shopArrayList
             ) {
            shopTemp.sellAllShopItems();
        }
    }
